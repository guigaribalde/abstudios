...
export const Shipment = pgTable('shipment', t => ({
  ...
  status: t.text({ enum: ['pending', 'shipped', 'delivered', 'cancelled'] }).notNull(),
  createdAt: t.timestamp().defaultNow().notNull(),
  updatedAt: t
    .timestamp({ mode: 'date', withTimezone: true })
    .$onUpdateFn(() => new Date()),
...
export const SchoolRelations = relations(School, ({ many }) => ({ users: many(User), shipments: many(Shipment) }));
